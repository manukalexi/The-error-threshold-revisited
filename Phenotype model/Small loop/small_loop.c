...
char *master= "UGGACCCCAAUGGCAAGGGGGAAUU\0"; //RNA sequence of the master of length L containing a small loop in its secondary structure and the active region in a previously defined position
int a = ((L-1)/2)-5; //start base of loop (with neck) -> counting from 0!
int b = ((L-1)/2)+4; //end base of loop (with neck) -> counting from 0!
...
int type(int i){
    ...
    char *functions[] = {"(......)","(.......)","(........)","(.........)","(..........)"}; //defining different loop lengths
    ...
    for (int size = 10; size >= 6; size--) {
        ...
        for(j = loop_start; j <= loop_end; j++){ 
            if(structure[j] == functions[size - 6][j - loop_start]){ 
                sum++;
            }
        }
...
double fit(int i){
    ...
    //no acceptable secondary structure is present
    if(fit1 == 5){ 
        f=1; //baseline fitness -> junk!
        free(structure);
        return f;
...
int main(int argc, char** argv){
    ...
    for(nmu = 0; nmu < 200; nmu = nmu+1){
        mu = 0.000 + 0.001 * nmu;
        sprintf(c, "L%d_n%.1lf_small_loop_REP1-%d_%lf.txt", L, n, REP, mu);
        output = fopen(c, "wt");
        coex = 0;
        ext = 0;
        aveave = 0.0;
