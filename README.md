...
- **Phenotype model:** the average master concentration (`ave`) over the last $10^6$ generations for each parallel simulation. The final line contains mutation rate `mu`, total average `aveave`, number of parallel simulations resulting in master survival `coex`, and number of parallel simulations resulting in master extinction `ex`.
